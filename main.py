...
def getChannelData(channelid):
    ...
    return [
        [
            ...
            "author_banner": urllib.parse.quote(t["authorBanners"][0]["url"]) if len(t['authorBanners']) else '',
...
def channel(channelid:str, response: Response, request: Request, yuki: Union[str] = Cookie(None), proxy: Union[str] = Cookie(None)):
    ...
    return template("channel.html", {"request": request, "results": t[0], "channel_name": t[1]["channel_name"], "channel_icon": t[1]["channel_icon"], "channel_profile": t[1]["channel_profile"], "cover_img_url": t[1]["author_banner"], "proxy": proxy})
